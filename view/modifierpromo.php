      ...
      <div class="page-wrapper">
        ...
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb p-0">
            ...
            <li class="breadcrumb-item" aria-current="page">Modifier Promotion</li>
          </ol>
        </nav>
...
$ress= $listeser->fetchall();
	...
	if (isset($_GET['idpromotion'])) {
		$promoC = new promoC();
		$result = $promoC->recupererpromotion($_GET['idpromotion']);
		
		foreach ($result as $row) {
			
			$services = $row['idservice'];
		    $mydd = date("d-m-Y", strtotime($row['datedeb']));
		    
			$mydf = date("d-m-Y", strtotime($row['datefin']));
			$promo= $row['valeur'];
			?>
			<form method="POST" action="traitmodifier.php">
				<table  border="0" width="0" cellpadding="10"   align="center">
					<tbody align="center">
					...
					<tr>
		              ...
		              ?>
	             	<option value ="<?php echo $ress[$i]['nomservice']?>" <?php if($ress[$i]['nomservice']==$services) {echo ('selected');}?>  > <?php echo $ress[$i]['nomservice']?></option>
			<?php } ?>
                       </select> </td>
					
					
						<td>Start Date: </td>
						<td width="15%"><input type="date" name="mydd" style="margin-left:0%; margin-top: 0%;" value="<?PHP echo $row['datedeb'] ?>" /></td>
					
						<td>End Date :</td>
						<td  width="15%"><input type="date" name="mydf" style="margin-left:0%; margin-top: 0%;" value="<?PHP echo $row['datefin']?>" /></td>
					
				
						<td>promotion: </td>
						<td ><input type="text" name="promo" value="<?PHP echo $promo ?>"> %</td>
					
            <td>
            <button type="submit" name="modifier" value="modifier" class="mb-1 btn btn-outline-danger">
                  modify</button></td>
					
					</tr>
					<tr>
						<td></td>
						<td><input type="hidden" name="idp_mod" value="<?PHP echo $_GET['idpromotion']; ?>"></td>
					</tr>
					   </tbody></table>
			</form>
	<?PHP
